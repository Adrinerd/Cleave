...
## Modo online (nuevo en la v4)

Cada uno en su móvil. Uno crea la sala, el otro entra con un código de 4 letras.

1. Los dos abrís la misma dirección.
2. Uno pulsa **Online → Crear partida**, escribe su nombre y le sale un código tipo `K4RM`.
3. El otro pulsa **Online → Tengo un código**, escribe su nombre y el código.
4. Cada uno reparte su paladar en su propio móvil. Cuando los dos estáis listos, empieza.

**La conexión es directa entre los dos móviles (WebRTC).** No hay servidor guardando nada, no hace falta cuenta y no cuesta dinero.

**Tu paladar no sale nunca de tu móvil.** Solo se envía el valor de una carta en el instante en que la revelas — que es cuando pasa a ser pública igualmente. El otro dispositivo no puede saber lo que aún no has enseñado.

### Si no conecta

- En la **misma wifi** funciona casi siempre. Entre datos móviles distintos falla bastante más: el operador suele bloquear la conexión directa.
- Los dos tenéis que tener la **misma versión**. Si has actualizado el `index.html`, sube el número de `sw.js` y recargad los dos.
- La sala solo existe mientras **el que la creó** tenga la pestaña abierta.
- Si se corta a mitad de partida, se pierde. Es un prototipo, no un producto.

### Si quieres que sea fiable de verdad

La conexión directa depende de un intermediario público gratuito que a veces falla. Para una prueba abierta con desconocidos, cámbialo por **Firebase Realtime Database** (plan gratuito de sobra) o **Supabase Realtime**: son 15 líneas y funciona en cualquier red. Lo único que hay que sustituir es la sección `/* RED */` del `index.html`.
